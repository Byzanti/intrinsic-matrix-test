...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate, ItemSelectionViewControllerDelegate, AVCaptureVideoDataOutputSampleBufferDelegate /* Peter added delegate */ {
    ...
    override func viewDidLoad() {
        ...
        /* Start Peter add */
        //Overriding original code
        //You can't have both AVCaptureVideoDataOutput and the SaveFile one at the same time.
        //So here we are adding this as the output, at least until something else in the app kicks in later.
        
        print("Overriding")
        let peterSession = AVCaptureSession()
        ...
        peterSession.addInput(input)
        
        //This is the video output format we need in order to get the instrinc matrix, according to stack overflow.
        let captureVideoDataOutput = AVCaptureVideoDataOutput()
        captureVideoDataOutput.setSampleBufferDelegate(self, queue: videoQueue)
        peterSession.addOutput(captureVideoDataOutput)
        captureVideoDataOutput.connection(with: .video)?.isCameraIntrinsicMatrixDeliveryEnabled = true //Must be set after added to output!
        
        //Start running and assign to view
        peterSession.startRunning()
        previewView.session = peterSession
        
        
        /* End Peter add -- and uncomment the next line*/
    ...
    //Peter added - this is the bit where you get the intrinsic matrix
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if (runCount == 0) {
            print("Capturing buffer\n------------------------------------")
            print(sampleBuffer)
            print("-------------------------------------------")
            if let intrinsicMatrix = CMGetAttachment(sampleBuffer, key:kCMSampleBufferAttachmentKey_CameraIntrinsicMatrix, attachmentModeOut:nil) {
                //let matrix: matrix_float3x3 = camData.withUnsafeMutableBytes { $0.pointee }
                //print(matrix)
                print(intrinsicMatrix)
                let nsData = intrinsicMatrix as! Data
                
                print("Intrinsic matrix")
                let matrix: matrix_float3x3 = nsData.withUnsafeBytes { $0.pointee }
                print(matrix)
